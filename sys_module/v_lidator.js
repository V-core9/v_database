...
const v_lidator = async (cfg, data) => {
  ...
  if (typeof cfg.min !== 'undefined') if (len < cfg.min) errorList.push({ type: 'error', input_value: data, msg: cfg.msg.error.min, suggest: ' Hint: Minimum Length is [' + cfg.min + ']', confirm: cfg.confirm });
  if (typeof cfg.max !== 'undefined') if (len > cfg.max) errorList.push({ type: 'error', input_value: data, msg: cfg.msg.error.max, suggest: ' Hint: Maximum Length is [' + cfg.max + ']', confirm: cfg.confirm });
